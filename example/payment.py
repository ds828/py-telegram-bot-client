...
from simplebot.utils import parse_callback_data, pretty_print
...
def on_show_menu(bot: SimpleBot, message: Message):
    keyboard = MultiSelect(name="menu")
...
@router.callback_query_handler(callable_match=parse_callback_data, name="menu")
def on_select_button_click(bot, callback_query, *callback_data_args):
    menu_select = MultiSelect(
        name="menu", layout=callback_query.message.reply_markup.inline_keyboard
    )
    menu_select.toggle(callback_data_args)
    bot.edit_message_reply_markup(
        chat_id=callback_query.from_user.id,
        message_id=callback_query.message.message_id,
        reply_markup=InlineKeyboardMarkup(inline_keyboard=menu_select.layout),
    )
...
def on_submit(bot, callback_query):
    ...
    selected_options = menu_select.get_selected()
    ...
    bot.send_invoice(
        ...
        currency="AUD",
        prices=[LabeledPrice(label=option[0], amount=option[1]) for option in selected_options],
        need_name=True,
...
def on_shipping_query(bot: SimpleBot, shipping_query: ShippingQuery):
    ...
    bot.answer_shipping_query(
        ...
        shipping_options=(
            ShippingOption(
                id="1", title="delivery", prices=(LabeledPrice(label="delivery fee", amount=998),)
            ),
            ShippingOption(
                id="2", title="pick up", prices=(LabeledPrice(label="pick up", amount=0),)
            ),
        ),
    )
...
def on_pre_checkout_query(bot: SimpleBot, pre_checkout_query: PreCheckoutQuery):
