...
def on_reply_media_group(bot, message):
    ...
    document = InputMediaDocument(media=InputFile(
        "sample.txt", os.path.abspath("./sample/sample.txt")),
                                  thumb=thumb)
    audio = InputMediaAudio(media=InputFile(
        "sample.mp3", os.path.abspath("./sample/sample.mp3")),
                            thumb=thumb)
